      ...
      export default {
          ...
          data(){
            return{
              ...
              gridParam: {},
              sourceURL:URL.URL_HANDLEJOB_getHandledJobData,
              eventHander:{
                getData:function(currentPage,pageSize,query){//分页等各种需要加载数据的情况   如果有该方法则使用该方法获取数据
                  	components.handleJobPage.table = this;
                  let _this=components.handleJobPage;
                  	let gridParam = components.handleJobPage.gridParam;
                ...
                rowDBclick:function(row,event,table){
		            //跳转页面
		            components.handleJobPage.handleOpen(this, 'openForm');
	          	}
              },
              iData:{
                "prid": 'handleJobGrid',
              ...
              buttons:[
                ...
                },
                {
                  ...
                  onTap:function(ev){
                    ...
                    let listName = activeBusiness ==  null ? "已办工作" : activeBusiness.listName == undefined ? "已办工作" : activeBusiness.listName;
                    //let listName="已办工作";
          ...
          watch:{
            gridParam:{
              handler:function(newVal){
                getComponent('handleJobFileTable').context.refrushData()
              },
              deep:true
            }
          },
          ...
          methods:{
            init:function (data) {
              ...
              this.$http.fetch(URL.URL_HANDLEJOB_getHandledJobColumn+"?listName="+listName).then((res) => {
            ...
            handleOpen: function(funEvent, funtype){
            	let main = components.handleJobPage;
              	let row = null;
	                  	...
	                  	this.$confirm('当前选中'+taskIds.length+'项工作收回，是否继续？', '提示', {
	                    	...
	                    	if(taskIds.length==1){
		                      	...
		                      	this.$http.fetch(URL.URL_HANDLEJOB_removeBatchOperationCache, {"cacheKey":"batchWithdraw_"}).then((data) => {
			                        workFlow.setVue(components.handleJobPage);
            ...
            refrushData(){
            	var main=components.handleJobPage;
          		if(main.table)main.table.refrushData();
            }
          },
          mounted(){
            components.handleJobPage=this;
            Loaded.clearUnuse();
            Loaded.exec({}, "handleJobGrid");
          }
      }
  </script>
